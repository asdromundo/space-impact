...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[]) {
    ...
    SDL_Window* window = SDL_CreateWindow("Pong", windowStartWidth, windowStartHeight,
    ...
    *appstate = new AppContext{
    ...
    if (app->system_interface->LogMessage(
            Rml::Log::LT_INFO, Rml::CreateString("Using SDL renderer: %s", SDL_GetRendererName(app->renderer)))) {
    ...
    Rml::Context* context =
        Rml::CreateContext("main", Rml::Vector2i(windowStartWidth, windowStartHeight), app->render_interface);
    if (!context) {
